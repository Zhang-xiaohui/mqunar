    ...
    <div>
        <template v-for="(item, index) in menuList">
            <div style="text-align: center;" :key="index">
                <Dropdown transfer v-if="item.children.length !== 1 || (item.children.length == 1 && !item.hasSubMenu)" placement="right-start" :key="index" @on-click="changeMenu">
                ...
                <Dropdown transfer v-else placement="right-start" :key="index" @on-click="changeMenu">
                    <Button @click="changeMenu(item.children[0].name)" style="width: 70px;margin-left: -5px;padding:10px 0; color:white" type="text">
                        <!-- <Icon :size="20" :color="iconColor" :type="item.icon"></Icon> -->
                        <i class="iconfont" v-bind:class="item.icon"></i>
                    </Button>
                    <DropdownMenu style="width: 200px;" slot="list">
